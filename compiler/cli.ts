...
switch (cmd) {
  ...
  case "doctor": {
    ...
    const why = c.llm === "claude-cli" ? (found ? " (claude in PATH)" : " (claude not in PATH: run `claude login`, or set ANTHROPIC_API_KEY)") : needsKey[c.llm] ? (found ? ` (${needsKey[c.llm]} set)` : ` (set ${needsKey[c.llm]})`) : "";
