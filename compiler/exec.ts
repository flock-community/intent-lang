...
export async function runJobsIsolated(dir: string, target: string, jobs: Job[], timeoutMs = 120_000): Promise<(ExampleResult | TraceResult | ExploreResult)[] | { error: string }> {
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const res = await runJobs(dir, target, jobs);
